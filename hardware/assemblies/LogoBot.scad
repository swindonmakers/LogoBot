...
LogoBot_Con_UltrasonicLeft = [[30, BaseDiameter/2 - 12, dw], [0,0,-1], 25,0,0];
...
module LogoBotAssembly ( Bumpers=true, Shell=true, PenLift=false, Ultrasonics=false ) {
    ...
    assembly("assemblies/LogoBot.scad", "Final Assembly", str("LogoBotAssembly(PenLift=",PenLift,")")) {
        ...
        translate([0, 0, GroundClearance]) {
			...
			if (Ultrasonics) {
				step(12, "Fit the ultrasonic distance sensors") {
					view(t=[-6,7,19], r=[64,1,212], d=625);

					for (i=[0,1])
						mirror([i,0,0])
						attach( LogoBot_Con_UltrasonicLeft, HCSR04UltrasonicSensor_Con_TopEdge, ExplodeSpacing=20)
